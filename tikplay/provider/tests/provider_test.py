...
class MockRetriever(Retriever):
    ...
    def handles(self, url):
        TestProvider.handles_called = True
        return True
...
class TestProvider(object):
    ...
    def test_mock_provider(self):
        ...
        assert isinstance(self.provider.get("http://example.com"), Task)
        assert TestProvider.retriever_initialized
        assert TestProvider.handles_called
    ...
    def test_register_all_one(self):
        """Tests Provider.register_all() with one valid and one invalid retriever visible."""
        tikplay.provider.retrievers.__all__ = [tikplay.provider.retrievers.youtube, MockModule()]
        self.provider = Provider({}, True)
        assert len(self.provider.retrievers) == 1
